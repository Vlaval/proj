...
MovieClip.prototype.restartAllChildren = function (isRoot = true) {
    
    
    for (i in this.children){
        let chd = this.children[i];
       // if (chd.clearChildren)
           // chd.clearChildren();

        if (chd._loop)
            chd._loop();
        if (chd.restartAllChildren)
            chd.restartAllChildren(false);
    
        this.removeChildById(chd.id);
    }
    this.children = [];
    if (isRoot) {
        this.m_currentFrameNo = 0;
        let svgE = document.getElementById (this.id);
        let allG = svgE.getElementsByTagName('g');
        for (i = allG.length-1; i>=0; i--){
            let el = allG[i];
            var tok = el.getAttribute('token');
            if (tok != this.id) {
            //    console.log(tok , 'removed');
